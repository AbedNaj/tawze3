    ...
    <div x-data="{ editing: false, restock: false }" class="space-y-6 max-w-3xl mx-auto">
        ...
        <div x-show="!editing && !restock" class="bg-white p-6 rounded-lg shadow-md border border-gray-100">
            <div class="flex items-center justify-between mb-4">
                ...
                <x-button wire:navigate href="{{ route('admin.products.show', ['product' => $inventory->product_id]) }}"
                    :label="__('inventory.product_details')" />
            </div>
            ...
            <dl class="divide-y divide-gray-200 text-gray-700">
                ...
                <div class="py-2 flex justify-between">
                    ...
                    <dd>
                        <span
                            class="px-2 py-1 rounded text-white inline-block {{ $inventoryStatus ? 'bg-' . $inventoryStatus->color() . '-500' : 'bg-gray-400' }}">
                            {{ $inventoryStatus?->label() ?? __('inventory.not_available') }}
                        </span>
                    </dd>
                </div>
            </dl>
            ...
            <div class="mt-6 flex gap-4">
                <x-button @click="restock = true" :label="__('inventory.restock')" />
                ...
                @endphp
                <x-button href="{!! route('admin.inventory.transfer') . '?' . $query !!}" :label="__('inventory.assign_to_employee')" />

                <x-button @click="editing = true" :label="__('inventory.edit')" />
            </div>
        </div>
        ...
        <div x-cloak x-show="editing" class="bg-white p-6 rounded-lg shadow-md border border-gray-100">
            ...
            <form class="space-y-2" action="{{ route('admin.inventory.update', ['inventory' => $inventory->id]) }}"
                ...
                <x-input name="min_stock_alert" :value="$inventory->min_stock_alert" :label="__('inventory.min_stock_alert')" type="number" />

                <div class="flex items-center gap-4">
                    <x-button primary type="submit" :label="__('common.save')" />
        ...
        <div x-cloak x-show="restock" class="bg-white p-6 rounded-lg shadow-md border border-gray-100">
            ...
            <form class="space-y-2" action="{{ route('admin.inventory.restock', ['inventory' => $inventory->id]) }}"
                ...
                <x-input type="number" name="quantity" value="0" :label="__('inventory.product_quantity_product')" />

                <div class="flex items-center gap-4">
                    <x-button primary type="submit" :label="__('common.save')" />
                    <x-button type="button" @click="restock = false" :label="__('common.cancel')" />
                </div>
            </form>
        </div>
    </div>
