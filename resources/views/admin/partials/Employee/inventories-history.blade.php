 <div class="mb-8">
     <h3 class="text-xl font-semibold text-gray-800 mb-6 flex items-center">
         ...
         {{ __('employee.employee_inventory') }}
     </h3>
             ...
             @forelse ($inventories as $index => $item)
                 <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                     ...
                     <div class="grid grid-cols-3 gap-4">
                         <div>
                             <label
                                 class="block text-xs font-medium text-gray-500 mb-1">{{ __('employee.product_name') }}</label>
                             <div class="text-sm font-medium">{{ $item->product->name ?? '' }}</div>
                         </div>
                         <div>
                             <label
                                 class="block text-xs font-medium text-gray-500 mb-1">{{ __('employee.inventory.quantity') }}</label>
                         ...
                         <tr class="hover:bg-gray-50 transition-colors">
                             ...
                             <td class="px-4 py-4 text-sm">
                                 <span
                                    ...
                                    bg-{{ $inventoryStatus->color() }}-100
                                    text-{{ $inventoryStatus->color() }}-800
