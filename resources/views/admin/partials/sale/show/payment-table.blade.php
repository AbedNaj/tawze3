            ...
            @forelse ($payments as $index => $item)
                <div class="border border-gray-200 rounded-lg p-4 bg-gray-50">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            <label
                                class="block text-xs font-medium text-gray-500 mb-1">{{ __('sale.sale.paid_amount') }}</label>
                            <div class="text-sm font-medium">{{ $item->paid_amount }}</div>
                        </div>
                        <div>
                            <label
                                class="block text-xs font-medium text-gray-500 mb-1">{{ __('sale.sale.payment_method') }}</label>
                            <div class="text-sm font-medium">{{ $item->paymentMethod->name }}</div>
                        </div>
                        <div>
                            <label
                                class="block text-xs font-medium text-gray-500 mb-1">{{ __('sale.sale.payment_date') }}</label>
            ...
            <table class="w-full min-w-full border-collapse">
                ...
                <thead class="bg-gradient-to-l from-gray-50 to-gray-100">
                    <tr>
                        ...
                        <th
                            class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">
                            {{ __('sale.sale.paid_amount') }}</th>
                        <th
                            class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">
                            {{ __('sale.sale.payment_method') }}</th>
                        <th
                            class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">
                            {{ __('sale.sale.payment_date') }}</th>
